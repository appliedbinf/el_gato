...
If available, we recommend using raw or trimmed reads instead of assemblies as this tends to produces more accurate results. When run with reads, el_gato is able to use read quality and coverage information to apply quality control rules. When run using assemblies, el_gato is unable to identify sequencing errors that have been incorporated into the assembly and may therefore report incorrect results.
