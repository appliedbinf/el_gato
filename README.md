...
Using the three reference sequences described above, reads from all samples we have tested map well to only one reference (although due to the high sequence diversity of *neuA*, mapping is performed with a mismatch penalty of 1, instead of the default 4). The reference sequence with the best mapping is identified using `samtools coverage`; only those reference loci with coverage over 99% (some alleles contain small indels so 100% is too strict) are retained for future processing. Once the best reference sequence (or sequences if two map with sufficient coverage) is identified, processing is the same as described above.
